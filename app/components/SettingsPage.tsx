          ...
          <div className="bg-white/5 backdrop-blur border border-white/10 rounded-xl p-6">
            ...
            <div className="space-y-4">
              ...
              <div className="p-3 bg-white/5 rounded-lg space-y-3">
                <label className="flex items-center justify-between cursor-pointer">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-white/40 text-sm">Get notified when an order has low profit</p>
                    </div>
                  </div>
                  <input
                    type="checkbox"
                ...
                {settings.email_profit_alerts && (
                  <div className="ml-13 pl-13">
                    <label className="text-white/60 text-sm block mb-2">Alert when profit is below:</label>
                    <div className="flex items-center gap-2">
                      <span className="text-white/60">$</span>
                      <input
                        type="number"
                        value={settings.email_alert_threshold || 0}
                        onChange={(e) => setSettings({ ...settings, email_alert_threshold: parseFloat(e.target.value) || 0 })}
                        className="w-24 px-3 py-1.5 bg-white/10 border border-white/20 rounded-lg text-white text-sm focus:outline-none focus:border-emerald-500"
                      />
                      <span className="text-white/40 text-sm">(0 = any loss)</span>
                    </div>
                  </div>
                )}
              </div>
